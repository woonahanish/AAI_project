...
class LinkedList_Var{
    ...
    int checkForInstantiation(string clause, Node_Var *head_ins){
...
class LinkedList_Clause_Var_List{
    ...
    void search(int C1, Node_Clause_var_List* head_clause, Node_Var *head_ins){
        Node_Clause_var_List* temp = head_clause;
        ...
        for(int i_s=0; i_s<4; i_s++){
            int ins_ckeck = l_for_instantiate.checkForInstantiation(temp->clause, head_ins);
            temp = temp->next;
        }
...
*/
void update_VL(int C1, Node_Clause_var_List *head_clause, Node_Var *head_ins){ 
    LinkedList_Clause_Var_List l_ins;
    
    l_ins.search(C1, head_clause, head_ins);
//    validate_R1(a, conclusion);
}
void rule_to_clause(int R1, Node_Clause_var_List *head_clause, Node_Var *head_ins){
    int C1;

    int clause_number = 4*(R1/10 -1) +1;
    cout<<"Clause Number "<<clause_number<<endl;
    update_VL(clause_number, head_clause, head_ins);
}
...
void search_con(Node* h, string variable, Node_Clause_var_List *head_clause, Node_Var *head_ins){
    ...
    rule_to_clause(rule_number, head_clause, head_ins);
}
